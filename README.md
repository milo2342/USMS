...
The bot automatically selects the **highest-numbered available badge** in `Personnel Database` column B where the Discord ID column I is empty. It then writes the member's Discord ID to I, RP name to the configured RP-name column (default D), join date to F, promotion date to G, sets K to `Active`, and places the badge number into the selected Personnel Roster G slot.
...
### Promotions
`/promote user rank [reason]` promotes one selected Marshal. `/mass promotions` and `/mass demotions` let you select the individual users directly (up to 10) and the bot reads each selected user’s current rank automatically.


## Personnel Database RP Name
The `/onboard` RP name is written to **column D** of `Personnel Database`.


### Role handling
Rank assignments use only existing server roles. The bot no longer uses broad fuzzy matching that can mistake `Director` for `Deputy Director`; rank roles are resolved by exact normalized names and explicit aliases. On promotion/demotion, other roster-rank roles are removed before the new rank role is assigned. On termination/resignation, the member's rank/support roles are removed and the department call-sign is cleared from the Discord nickname.


`/promotion` promotes one selected member. `/promotion-logs` configures the promotion log channel. `/termination-log channel:#channel` sets the termination log channel. `/terminate` automatically posts the termination log there.
