...
function normalizeRoleLookup(name) {
  return String(name ?? "")
    .toLowerCase()
    .replace(/^ms\s*[-|]\s*/i, "")
    .replace(/\s+/g, " ")
...
function findSimilarRole(guild, targetName) {
  const key = normalizeRoleLookup(targetName);
  const aliases = {
    "director": ["director"],
    ...
    "pre command": ["pre command", "command in training"]
  };
  for (const candidate of (aliases[key] || [key])) {
    const normalized = normalizeRoleLookup(candidate);
    const role = guild.roles.cache.find(r => !r.managed && normalizeRoleLookup(r.name) === normalized);
    if (role) return role;
  }
  return null;
}
...
function rankMatches(rank, candidate) {
  return normalizeName(rank) === normalizeName(candidate);
}
...
async function applyRankRole(guild, member, newRank, oldRank = "") {
  ...
  if (!newRole) {
    throw new Error(`I could not find an existing Discord role for **${newRank}**. No role was created.`);
  }

  // Remove any existing department rank role that is different from the new rank.
...
async function setMemberNickname(member, callsign, rpName = "") {
  ...
  const fallback = member.nickname || member.user?.displayName || "Member";
  const cleanedName = clean(rpName) || (fallback.includes("|") ? fallback.split("|").slice(1).join("|").trim() : fallback);
  await member.setNickname(`${callsign} | ${cleanedName}`.slice(0, 32)).catch(err => console.error("Nickname update failed:", err));
}
...
async function sendPromotionLog(guild, embed) {
  const cfg = guildSettings(guild.id);
  const ch = await configuredChannel(guild, cfg.promotionChannelId);
  if (!ch) {
    console.warn(`No promotion log channel configured for guild ${guild.id}.`);
    return false;
  }
  try {
    await withTimeout(ch.send({ embeds: [embed] }), 10000, "promotion log send");
    return true;
  } catch (err) {
    console.error("Promotion log send failed:", err);
    return false;
  }
}
...
await setCells(C.databaseSheet, {
  ...
  const dmSent = await dm(member, embed);
  if (!dmSent) console.warn(`Could not DM onboarded member ${member.id}.`);
  await sendAudit(i.guild, embed);

  if (i.channel?.isTextBased?.()) {
    await i.channel.send({ content: `✅ Successfully onboarded ${member} to **${DEPARTMENT}**.` }).catch(err => console.error("Onboard confirmation failed:", err));
  }

  await sendPromotionLog(i.guild, embed);
...
async function handleMove(i) {
  ...
  await setMemberNickname(member, slot.callsign, state.db.rpName);
  const actor = `<@${i.user.id}>`;
  ...
  const dmSent = await sendDM(member, embed);
  if (!dmSent) console.warn(`Could not DM moved/promoted member ${member.id}.`);
  await sendAudit(i.guild, embed);
  await sendPromotionLog(i.guild, embed);
  await i.editReply({embeds:[embed]});
}
...
async function updateMass(i, sub, fromRank, toRank, reason) {
  ...
  const actor=`<@${i.user.id}>`; const embed=actionEmbed(`Mass ${sub === "promotions" ? "Promotion" : "Demotion"}`,COLORS.purple,`Processed **${done}** member(s).`,actor,[{name:"From",value:fromRank,inline:true},{name:"To",value:toRank,inline:true},{name:"Skipped",value:String(skipped),inline:true},{name:"Reason",value:safeText(reason||"No reason provided.")}]); await sendAudit(i.guild,embed); if(sub === "promotions"){await sendPromotionLog(i.guild,embed);} else {await sendTypedLog(i.guild,"demotion",embed);} return {count:done,skipped};
}
...
client.on("interactionCreate",async i=>{
  try {
    ...
    if(cmd === "termination-log"){
      ...
      await saveSettings();
      return i.editReply({content:`✅ Terminations will now be logged to ${ch}.`});
    }
