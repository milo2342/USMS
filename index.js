...
function guildSettings(guildId) {
  settings.guilds[guildId] ||= { logRoleId: "", auditChannelId: "", promotionChannelId: "", logChannels: {}, ftoRoleId: "", commandPermissions: {} };
...
const RANK_ROLE_ALIASES = {
  "director": ["director"],
  "deputy director": ["deputy director"],
  "assistant director": ["assistant director"],
  "chief marshal": ["chief marshal"],
  "deputy chief marshal": ["deputy chief marshal"],
  ...
  "deputy marshal in charge": ["deputy marshal in charge"],
  "assistant deputy marshal in charge": ["assistant deputy marshal in charge"],
  "supervisory deputy marshal": ["supervisory deputy marshal"],
  "special deputy marshal": ["special deputy marshal"],
  "senior deputy marshal": ["senior deputy marshal"],
  "deputy marshal": ["deputy marshal"],
  "probationary deputy marshal": ["probationary deputy marshal"],
  "high command": ["heads", "high command"],
  "low command": ["low command"],
  "supervisor": ["supervisor"],
  "command in training": ["pre command", "command in training"],
  "pre command": ["pre command"]
};

function findSimilarRole(guild, targetName) {
  const target = normalizeRoleName(targetName);
  const candidates = RANK_ROLE_ALIASES[target] || [target];

  for (const candidate of candidates) {
    const role = guild.roles.cache.find(r =>
      !r.managed && normalizeRoleName(r.name) === candidate
    );
    if (role) return role;
  }
...
const SUPPORT_RANK_ROLES = [
  { roleName: "MS - Pre Supervisor", ranks: [
    "Supervisor", "Low Command", "Pre Command", "High Command",
    "Chief of Staff", "Director"
  ]},
  { roleName: "MS - Low Command", ranks: [
    "Low Command", "Pre Command", "High Command", "Chief of Staff", "Director"
  ]},
  { roleName: "MS - Pre Command", ranks: [
    "Pre Command", "High Command", "Chief of Staff", "Director", "Command in training"
  ]},
  { roleName: "MS - Heads", ranks: [
    "High Command", "Chief of Staff", "Director"
  ]},
  { roleName: "MS - Chief of staff", ranks: [
    "Chief of Staff", "Director"
  ]},
  { roleName: "MS - Director", ranks: [
    "Director"
  ]}
];

function rankMatches(rank, candidate) {
  return normalizeRoleName(rank) === normalizeRoleName(candidate);
}
...
async function applySupportRankRole(guild, member, rank) {
  for (const mapping of SUPPORT_RANK_ROLES) {
    ...
    const shouldHave = mapping.ranks.some(r => normalizeRoleName(r) === normalizeRoleName(rank));

    if (shouldHave && !member.roles.cache.has(role.id)) {
      await member.roles.add(role).catch(err => console.error(`Failed to add support role ${role.name}:`, err));
    } else if (!shouldHave && member.roles.cache.has(role.id)) {
      await member.roles.remove(role).catch(err => console.error(`Failed to remove support role ${role.name}:`, err));
    }
  }
}
...
const commands = [
  ...
  new SlashCommandBuilder().setName("promotion").setDescription("Promote an individual Marshal.")
    .addUserOption(o=>o.setName("user").setDescription("Discord member").setRequired(true))
...
function buildLogEmbed(type, target, actor, reason, details) {
  ...
  const styles = {
    removal: { color: COLORS.red, title: "USM • Removal Log", icon: "🗑️" },
    demotion: { color: COLORS.red, title: "USM • Demotion Log", icon: "📉" },
    transfer: { color: COLORS.blue, title: "USM • Transfer Log", icon: "🔄" },
    task: { color: COLORS.purple, title: "USM • Command Task Log", icon: "📋" },
    inactivity: { color: COLORS.gold, title: "USM • Inactivity Warning Log", icon: "⚠️" },
    loa: { color: COLORS.gold, title: "USM • LOA Log", icon: "🟡" },
    supervisorInterview: { color: COLORS.blue, title: "USM • Supervisor Interview Log", icon: "🎙️" },
    ftaInterview: { color: COLORS.blue, title: "USM • FTA Interview Log", icon: "🎙️" },
    training: { color: COLORS.green, title: "USM • Training Log", icon: "🎓" }
  };

  const style = styles[type] || { color: COLORS.black, title: `USM • ${meta.label} Log`, icon: "📝" };
...
async function handleAdd(i) {
  ...
  if (!slot) {
    throw new Error(`No open ${rank} slot with a valid callsign exists in Personnel Roster.`);
  }
  ...
  const welcomeEmbed = actionEmbed(
    "🎉 Welcome to USM",
    COLORS.green,
    `You have been onboarded to **United States Marshals**.`,
    actor,
    [
  ...
  const welcomeText =
    `🎉 **Welcome to USM**\n` +
    `You are now a Member Of the United States Marshals.\n\n` +
    `**Call Sign:** ${slot.callsign}\n` +
    `**Badge Number:** ${db.badge}\n\n` +
    `Please read this message in its entirety to get started on roleplaying as a Deputy.\n\n` +
    `USM operates at the highest level of realism and professionalism. For more information read:\n` +
    `https://discord.com/channels/1543149025526808616/1543149028152188960\n\n` +
...
async function handleTerminateResign(i, type) {
  ...
  await sendDM(member, actionEmbed(
    `USM • ${status}`,
    COLORS.red,
    `Your USM status has been changed to **${status}**.\n\n**Reason:** ${reason}`,
    actor,
    [
...
client.on("guildMemberAdd", async member => {
  try {
    ...
    const embed = actionEmbed(
      "🇺🇸 Welcome to WCRP | United States Marshals",
      COLORS.black,
      "**USM Deputy Program**",
      ...
      [
        {
          name: "Welcome",
          value: "Hello, welcome to the United States Marshals! If you have any questions head to https://discord.com/channels/1543149025526808616/1543149030715039749",
          inline: false
        },
        {
          name: "Before Going On Duty",
          value:
            "Before you're eligible to go on duty, **You Must complete a training and ride-alongs are required** to go in RTO, so head here to get them:\n" +
            "https://discord.com/channels/1543149025526808616/1543149030715039752\n" +
            "https://discord.com/channels/1543149025526808616/1543149034229866514\n" +
            "https://discord.com/channels/1543149025526808616/1543149034229866515\n" +
            "https://discord.com/channels/1543149025526808616/1543149034573926450\n" +
            "https://discord.com/channels/1543149025526808616/1543149034573926451",
          inline: false
        },
        {
          name: "New Information / Events",
          value:
            "To be notified about any new info/events within USM, head to https://discord.com/channels/1543149025526808616/1543149029024731182 — here you can also find the United States Marshals Vehicle/Uniforms Documents!",
          inline: false
        },
        {
          name: "Final Step",
          value: "Make sure you check out https://discord.com/channels/1543149025526808616/1543982541994991707",
          inline: false
        }
      ]
    );
