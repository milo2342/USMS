...
await setCells(C.databaseSheet, {
  ...
  const reason = "Initial onboarding";
  const logLine = `**${member} --> ${rank} | ${reason} | ${joinDate} | Approved By: ${approvedBy}**`;
  ...
  const welcomeEmbed = actionEmbed(
    "Welcome to San Andreas State Marshals",
    COLORS.green,
    `Welcome ${member}! You have been successfully onboarded as **${rank}**.`,
    actor,
    [
      { name: "RP Name", value: rpName || "—", inline: true },
      { name: "Call Sign", value: slot.callsign || "—", inline: true },
  ...
  const dmSent = await dm(member, welcomeEmbed);
  ...
  if (!promoCh) throw new Error("Promotion log channel has not been configured or could not be accessed. Use /promotion-logs first.");
  await withTimeout(promoCh.send({ embeds: [embed] }), 10000, "promotion log send");
...
async function handleMove(i) {
  ...
  const actor = `<@${i.user.id}>`;
  const promotionDate = today();
  const logLine = `**${member} --> ${newRank} | ${reason} | ${promotionDate} | Approved By: ${actor}**`;
  const embed = actionEmbed("Marshal Promotion", COLORS.green, logLine, actor, [
    {name:"Previous Rank",value:oldRank,inline:true},
    {name:"New Rank",value:newRank,inline:true},
    {name:"Call Sign",value:slot.callsign,inline:true},
    {name:"Badge",value:state.badge||"—",inline:true}
  ]);
  const dmSent = await sendDM(member, embed);
  if (!dmSent) console.warn(`Could not DM promoted member ${member.id}.`);
  const promoCh = await configuredChannel(i.guild, guildSettings(i.guildId).promotionChannelId);
  if (!promoCh) throw new Error("Promotion log channel has not been configured or could not be accessed. Use /promotion-logs first.");
  await withTimeout(promoCh.send({embeds:[embed]}),10000,"promotion log send");
  await sendAudit(i.guild, embed);
