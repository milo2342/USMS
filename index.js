...
const LOG_TYPES = {
  ...
  termination: {label: "Termination", key: "termination"}
};
...
function findSimilarRole(guild, targetName) {
  const aliases = new Map([
    ["director", ["MS - Director", "MS Director", "Director"]],
    ["assistant director", ["MS - Assistant Director", "MS Assistant Director", "Assistant Director"]],
    ["chief marshal", ["MS - Chief Marshal", "MS Chief Marshal", "Chief Marshal"]],
    ["chief of staff", ["MS - Chief of staff", "MS - Chief of Staff", "MS Chief of Staff", "Chief of Staff"]],
    ["deputy director", ["MS - Deputy Director", "MS Deputy Director", "Deputy Director"]],
    ["deputy chief marshal", ["MS - Deputy Chief Marshal", "MS Deputy Chief Marshal", "Deputy Chief Marshal"]],
    ["assistant deputy marshal in-charge", ["MS - Assistant Deputy Marshal in-Charge", "Assistant Deputy Marshal in-Charge"]],
    ["assistant deputy marshal in charge", ["MS - Assistant Deputy Marshal in Charge", "Assistant Deputy Marshal in Charge"]],
    ["deputy marshal in charge", ["MS - Deputy Marshal in Charge", "Deputy Marshal in Charge"]],
    ["deputy marshal", ["MS - Deputy Marshal", "MS Deputy Marshal", "Deputy Marshal"]],
    ["deputy marshals", ["MS - Deputy Marshals", "Deputy Marshals"]],
    ["district chief", ["MS - District Chief", "District Chief"]],
    ["district commander", ["MS - District Commander", "District Commander"]],
    ["division commander", ["MS - Division Commander", "Division Commander"]],
    ["high command", ["MS - Heads", "MS Heads", "High Command"]],
    ["low command", ["MS - Low Command", "MS Low Command", "Low Command"]],
    ["senior deputy marshal", ["MS - Senior Deputy Marshal", "Senior Deputy Marshal"]],
    ["senior deputy marshals", ["MS - Senior Deputy Marshals", "Senior Deputy Marshals"]],
    ["special deputy marshal", ["MS - Special Deputy Marshal", "Special Deputy Marshal"]],
    ["special deputy marshals", ["MS - Special Deputy Marshals", "Special Deputy Marshals"]],
    ["supervisor", ["MS - Supervisor", "Supervisor"]],
    ["watch commander", ["MS - Watch Commander", "Watch Commander"]],
    ["probationary deputy marshal", ["MS - Probationary Deputy Marshal", "Probationary Deputy Marshal"]],
    ["probationary deputy marshals", ["MS - Probationary Deputy Marshals", "Probationary Deputy Marshals"]],
  ]);
  const target = normalizeName(targetName);
  const candidates = aliases.get(target) || [targetName];
  for (const candidate of candidates) {
    const normalized = normalizeName(candidate);
    const exact = guild.roles.cache.find(r => !r.managed && normalizeName(r.name) === normalized);
...
async function applyRankRole(guild, member, newRank, oldRank = "") {
  ...
  if (!newRole) throw new Error(`I could not find an existing Discord role for **${newRank}**. No new role was created.`);

  // Remove any existing rank role that corresponds to a different roster rank.
  const liveRanks = await liveRanks();
...
const commands = [
  ...
  new SlashCommandBuilder().setName("promotion-logs").setDescription("Set the channel where promotion events are posted.")
    .addChannelOption(o=>o.setName("channel").setDescription("Promotion log channel").setRequired(true)),
  new SlashCommandBuilder().setName("termination-log").setDescription("Set the channel where termination logs are posted.")
    .addChannelOption(o=>o.setName("channel").setDescription("Termination log channel").setRequired(true)),
...
async function handleAdd(i) {
  ...
  // Public confirmation in the channel where /onboard was run.
  if (i.channel?.isTextBased?.()) {
    await i.channel.send({ content: `✅ Successfully onboarded ${member} to **${DEPARTMENT}**.` }).catch(() => {});
  }
...
async function handleTerminateResign(i, type) {
  ...
  // Remove all supervisory/command helper roles on separation as well.
  for (const mapping of SUPPORT_RANK_ROLES) {
  ...
  // Remove the department call-sign from the Discord nickname.
  if (member.manageable) await member.setNickname(null).catch(() => {});
  const actor = `<@${i.user.id}>`; const embed = actionEmbed(`Marshal ${status}`, COLORS.red, `${member}`, actor,[{name:"Badge",value:state.badge||"—",inline:true},{name:"Previous Rank",value:state.roster?.rank||"—",inline:true},{name:"Reason",value:reason}]); await sendDM(member,embed); await sendTypedLog(i.guild,"termination",embed); await i.editReply({embeds:[embed]});
}
...
client.on("interactionCreate",async i=>{
  try {
    ...
    if(cmd === "termination-log"){
      if(!isOwner(i)) throw new Error("Only bot owners can configure the termination log channel.");
      const ch=i.options.getChannel("channel",true);
      guildSettings(i.guildId).logChannels.termination=ch.id;
      await saveSettings();
      return i.editReply({content:`✅ Termination logs will now go to ${ch}.`});
    }
