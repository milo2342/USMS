...
async function liveRanks(prefix = "") {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (rank && (!p || rank.toLowerCase().includes(p))) seen.add(rank);
  }
  return [...seen].sort((a, b) => a.localeCompare(b));
}
async function liveBadges(prefix = "") {
  const rows = await dbRows();
  const seen = new Set(); const p = clean(prefix).toLowerCase();
  for (let i = 1; i < rows.length; i++) {
    const badge = clean(cell(rows[i], C.dbBadgeCol)); const discordId = clean(cell(rows[i], C.dbDiscordIdCol));
    if (badge && !discordId && (!p || badge.toLowerCase().includes(p))) seen.add(badge);
  }
...
const commands = [
  new SlashCommandBuilder().setName("add").setDescription("Add a new Marshal from the Personnel Database.")
    ...
    .addStringOption(o => o.setName("badge").setDescription("Badge number from Personnel Database column B").setAutocomplete(true).setRequired(true)),
  ...
  new SlashCommandBuilder().setName("ranks").setDescription("List ranks live from Personnel Roster."),
...
async function handleAdd(i) {
  ...
  const badge = clean(i.options.getString("badge", true));
  if (!member) throw new Error("That member is not in this server.");
  if (await findDatabaseByDiscordId(member.id)) throw new Error("That Discord user is already assigned in Personnel Database column I.");
  const db = await findDatabaseByBadge(badge);
  if (!db) throw new Error(`Badge ${badge} was not found in Personnel Database column B.`);
  if (clean(cell(db.values, C.dbDiscordIdCol))) throw new Error(`Badge ${badge} is already assigned to another Discord ID.`);
  const slot = await findOpenRosterSlot(rank);
  if (!slot) throw new Error(`No open ${rank} slot with a valid 1Z-## call-sign exists in Personnel Roster.`);
  await setCells(C.databaseSheet, {
    [`${C.dbDiscordIdCol}${db.row}`]: member.id,
    [`${C.dbJoinDateCol}${db.row}`]: today(),
    [`${C.dbPromotionDateCol}${db.row}`]: today(),
    [`${C.dbStatusCol}${db.row}`]: "Active",
    [`${C.dbTerminationCol}${db.row}`]: false,
    [`${C.dbResignedCol}${db.row}`]: false,
    [`${C.dbLoaCol}${db.row}`]: false
  });
  await setCell(C.rosterSheet, `${C.rosterBadgeCol}${slot.row}`, badge);
  await applyRankRole(i.guild, member, rank);
  await setMemberNickname(member, slot.callsign);
  const actor = `<@${i.user.id}>`;
  const embed = actionEmbed("Marshal Added", COLORS.green, `${member} has been added to the **${DEPARTMENT}**.`, actor, [
    {name:"Call Sign",value:slot.callsign||"—",inline:true},{name:"Badge",value:badge,inline:true},{name:"Rank",value:rank,inline:true},{name:"Status",value:"Active",inline:true},{name:"Join Date",value:today(),inline:true}
  ]);
...
client.on("interactionCreate",async i=>{
  try {
    if(i.isAutocomplete()){
      ...
      if((name==="add"||name==="move"||name==="mass") && focused?.name?.includes("rank")){
        const ranks=await liveRanks(value); return i.respond(ranks.slice(0,25).map(r=>({name:r,value:r})));
      }
      if(name==="add" && focused?.name==="badge"){
        const b=await liveBadges(value); return i.respond(b.slice(0,25).map(x=>({name:x,value:x})));
      }
      return i.respond([]);
    }
    if(i.isButton()){
